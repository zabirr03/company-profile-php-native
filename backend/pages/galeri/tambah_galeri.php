...
if(isset($_POST['bsimpan'])){
    ...
    //script untuk mengambil gambar
        move_uploaded_file($_FILES['tgambar']['tmp_name'], '../assets/backend/img_galeri/'.$rand.'_'.$filename); //pengambilan  Alamat file nya dihitung dari index

    $simpan = mysqli_query($koneksi, "INSERT INTO galeri (judul, gambar, deskripsi) 
                                      VALUES ('$_POST[tjudul]','$gambar','$_POST[tdeskripsi]' )");

    if($simpan){

        echo "<script> alert('Simpan Data Sukses');document.location='index.php?page=tampil_galeri';</script>";
    }else{
        echo "<script> alert('Simpan Data Gagal')document.location='index.php?page=tambah_galeri';</script>";
    }
}
